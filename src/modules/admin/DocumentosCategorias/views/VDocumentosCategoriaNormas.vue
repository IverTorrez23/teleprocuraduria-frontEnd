...
const loadDocCategorias = async (event?: DataTableSortEvent) => {
  ...
  const response = await documentoCategoriaService.getDocCategoriasNormas(opciones)
...
const saveDocCategoria = async () => {
  ...
  try {
    if (docCategoria.value.id) {
      ...
      docCategoria.value.tipo = TipoDocumento.NORMAS //Por defecto su tipo es NORMAS
      ...
      <div class="card">
        ...
        >
          <template #header>
            ...
            >
              <h4 class="m-0">Administrar Categoría de Normas</h4>
