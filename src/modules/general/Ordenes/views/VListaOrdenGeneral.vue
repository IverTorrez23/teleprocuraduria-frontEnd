...
const loadOrden = async (event?: DataTableSortEvent) => {
  try {
    ...
    const message = error || 'Ocurrió un error al cargar las órdenes.'
...
const totalSumaEgresos = (): string => {
  ...
  ordenes.value.forEach((orden) => {
    if (orden.final_costos && orden.final_costos.total_egreso !== null) {
      const egreso = Number(orden.final_costos.total_egreso) || 0
      const propina = Number(orden.propina) || 0
      total += egreso + propina
    }
  })
  return `${total.toFixed(2)}`
}
    ...
    >
      <ColumnGroup type="header">
        <Row>
          ...
          <Column header="FINANZAS" :colspan="6" />
        ...
        <Row>
          ...
          <Column header="Costo Procuraduria (Bs.)" :colspan="3" />
          <Column header="" />
        </Row>
        <Row>
          ...
          <Column header="Propina" field="propina" />
      ...
      <Column field="propina">
        <template #body="slotProps">
          {{ slotProps.data?.propina }}
        </template>
      </Column>
      <Column field="final_costos.total_egreso">
        <template #body="slotProps">
          {{
            slotProps.data?.final_costos?.total_egreso != null && slotProps.data?.propina != null
              ? (+slotProps.data.final_costos.total_egreso + +slotProps.data.propina).toFixed(2)
              : 'No definido'
          }}
        </template>
      </Column>
