...
const loadOrdenPorUrgencias = async () => {
  const result = await gestionOrdenService.listadoPorUrgencias()
...
const actualizarContador = (orden: IGestionOrden) => {
  const ahora = dayjs();
  const fechaObjetivo = dayjs(orden.fecha_fin);
  ...
  if (diff > 0) {
    ...
    orden.contador = `${dias}d ${horas}h ${minutos}m ${segundos}s`;
  } else {
    orden.contador = "Vigencia finalizada";
  }
};
const tick = () => {
  gestionOrden.value.forEach((orden) => actualizarContador(orden));
};
</script>
<template>
  <OBotonesSeguimiento />
      ...
      <template #header>
        <div>
          <div class="flex flex-wrap gap-2 align-items-center justify-content-between">
            <h4 class="m-0">Listado Por Urgencias</h4>
