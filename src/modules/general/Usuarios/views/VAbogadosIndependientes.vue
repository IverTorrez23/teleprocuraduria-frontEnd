...
const loadAbogadosIndependientes = async (event?: DataTableSortEvent) => {
  ...
  const response = await usuarioService.listarAbogadosIndPaginado(
   opciones
  )
...
onMounted(loadAbogadosIndependientes)

watch(
  () => filters.value.global.value,
  () => {
    loadAbogadosIndependientes()
  }
)
...
<template>
  <div class="card">
    ...
    <DataTable
      ...
      :autoLayout="true"
      :lazy="true"
      @page="loadAbogadosIndependientes"
      @sort="loadAbogadosIndependientes"
    >
      <template #header>
        <div class="flex flex-wrap gap-2 align-items-center justify-content-between">
          <h4 class="m-0">Abogados Independientes</h4>
