...
const loadCausas = async (event?: DataTableSortEvent) => {
  ...
  const response = await causaService.getCausasOrdenDineroEntregado(opciones)
      ...
      <Column field="codigo" header="Código" sortable style="min-width: 9rem">
        <template #body="slotProps">
          <Button
            ...
            link
            @click="$router.push({ name: 'OrdenesDineroEntregado', params: { idCausa: slotProps.data.id } })"
          />
        </template>
      </Column>
