...
const loadCausasSaldosActivos = async () => {
  const response = await causaService.listadoSaldosActivosCausas()

  if (response) {
    // Recorrer causas y agregar el dinero comprometido
    causasSaldosActivos.value = response
  } else {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Fallo  al obtener causas saldos activos',
      life: 3000
    })
  }
}
  ...
  <div class="grid">
    <div class="col-12">
      <div class="card">
        ...
        >
          <h4 class="m-1">Causas con Saldos Activos</h4>
          ...
          >
          <Column field="usuario.tipo" header="Tipo" sortable>
            <template #body="slotProps">
              <span class="p-column-title">Tipo</span>
              {{ slotProps.data.usuario.tipo }}
            </template></Column
          >
