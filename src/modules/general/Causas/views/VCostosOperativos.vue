...
const loadCausas = async (event?: DataTableSortEvent) => {
  ...
  const response = await CausaService.getCausasTodasCostosOperativos(opciones)
    ...
    >
      <template #header>
        <div class="flex flex-wrap gap-2 align-items-center justify-content-between">
          <h4 class="m-0">Costos Operativos</h4>
      ...
      <Column field="codigo" header="Código" sortable style="min-width: 9rem">
        <template #body="slotProps">
          <Button
            ...
            link   
            @click="$router.push({ path: '/detalle-costos-operativos/' + slotProps.data.id })"
          />
        </template>
      </Column>
      ...
      >
      <Column field="billetera" header="Saldo" style="min-width: 10rem">
        <template #body="slotProps">
          {{ slotProps.data.tiene_billetera === 1 ? slotProps.data.billetera : '' }}
        </template></Column
      >
