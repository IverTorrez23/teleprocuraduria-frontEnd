...
import { isAbogadoAutorizado, isAbogadoINDEPEN_DEPEN } from '@/common/utils/usuarioAutorizado'
...
const loadCausasPorCodigoLegal = async (event?: DataTableSortEvent) => {
  const page = event ? event.first / event.rows + 1 : pagination.value.currentPage
  const perPage = event ? event.rows : pagination.value.rowsPerPage
  const search: ISearch[] = filters.value.global.value
    ? [{ fields: ['nombre', 'observacion'], keyword: filters.value.global.value }]
  ...
  const response = await CausaService.getCausasPorCodigoLegal(
    opciones,
    codigoSeleccionadoFiltro.value ?? 0
  )
...
const loadCausasPorAbogado = async (event?: DataTableSortEvent) => {
  const page = event ? event.first / event.rows + 1 : pagination.value.currentPage
  const perPage = event ? event.rows : pagination.value.rowsPerPage
  const search: ISearch[] = filters.value.global.value
    ? [{ fields: ['nombre', 'observacion'], keyword: filters.value.global.value }]
  ...
  const response = await CausaService.getCausasPorAbogado(
    opciones,
    abogadoSeleccionadoFiltro.value ?? 0
  )
...
const loadCausasPorCategoria = async (event?: DataTableSortEvent) => {
  const page = event ? event.first / event.rows + 1 : pagination.value.currentPage
  const perPage = event ? event.rows : pagination.value.rowsPerPage
  const search: ISearch[] = filters.value.global.value
    ? [{ fields: ['nombre', 'observacion'], keyword: filters.value.global.value }]
  ...
  const response = await CausaService.getCausasPorCategoria(
    opciones,
    categoriaSeleccionadoFiltro.value ?? 0
  )
...
const loadCausasPorProcurador = async (event?: DataTableSortEvent) => {
  const page = event ? event.first / event.rows + 1 : pagination.value.currentPage
  const perPage = event ? event.rows : pagination.value.rowsPerPage
  const search: ISearch[] = filters.value.global.value
    ? [{ fields: ['nombre', 'observacion'], keyword: filters.value.global.value }]
  ...
  const response = await CausaService.getCausasPorProcurador(
    opciones,
    procuradorSeleccionadoFiltro.value ?? 0
  )
...
const filtradoPorCodigo = async () => {
  ...
  if (codigoSeleccionadoFiltro.value && codigoSeleccionadoFiltro.value > 0) {
    await loadCausasPorCodigoLegal()
  } else {
    await loadCausas()
  }
...
const filtradoPorProcurador = async () => {
  ...
  if (procuradorSeleccionadoFiltro.value && procuradorSeleccionadoFiltro.value > 0) {
    await loadCausasPorProcurador()
  } else {
    await loadCausas()
  }
  categoriaSeleccionadoFiltro.value = null
...
const filtradoPorCategoria = async () => {
  ...
  if (categoriaSeleccionadoFiltro.value && categoriaSeleccionadoFiltro.value > 0) {
    await loadCausasPorCategoria()
  } else {
    await loadCausas()
  }
  abogadoSeleccionadoFiltro.value = null
...
const filtradoPorAbogado = async () => {
  ...
  if (abogadoSeleccionadoFiltro.value && abogadoSeleccionadoFiltro.value > 0) {
    await loadCausasPorAbogado()
  } else {
    await loadCausas()
  }
  codigoSeleccionadoFiltro.value = null
...
const loadAbogados = async () => {
  ...
  const abogadoTodos = {
    ...
    updated_at: '',
    persona: null,
    nombreCompleto: `Todos los abogados`
  }
  abogados.value.unshift(abogadoTodos)
}
...
const loadProcuradores = async () => {
  ...
  if (result.data) {
    ...
    const soloUnProcurador = {
      ...
      created_at: '',
      updated_at: '',
      nombreCompleto: 'Todos los procuradores' // este campo adicional si lo necesitas para mostrar en el dropdown
    }

    procuradores.value.unshift(soloUnProcurador)
  } else {
    toast.add({
      severity: 'error',
      summary: 'Error',
    ...
    >
      <template #header>
        <div>
          ...
          <!-- Fila estática personalizada -->
          <div class="p-d-flex p-flex-row p-ai-center p-mt-3 p-mb-2" style="margin-top: 0.3rem">
            <Dropdown
              v-model="codigoSeleccionadoFiltro"
              :options="codigosCausa"
              optionLabel="codigos"
              optionValue="id"
              placeholder="Filtrar por Código"
              class="p-mr-2 dropdown-spacing"
              @change="filtradoPorCodigo"
            />
            <Dropdown
              v-if="usuario?.tipo != TipoUsuario.PROCURADOR"
              v-model="procuradorSeleccionadoFiltro"
              :options="procuradores"
              optionLabel="nombreCompleto"
              optionValue="id"
              placeholder="Filtrar por Procurador"
              class="p-mr-2 dropdown-spacing"
              @change="filtradoPorProcurador"
            />
            <Dropdown
              v-if="
              ...
              "
              v-model="abogadoSeleccionadoFiltro"
              :options="abogados"
              optionLabel="nombreCompleto"
              optionValue="id"
              placeholder="Filtrar por Abogado"
              class="p-mr-2 dropdown-spacing"
              @change="filtradoPorAbogado"
            />
            <Dropdown
              v-model="categoriaSeleccionadoFiltro"
              :options="categorias"
              optionLabel="nombre"
              optionValue="id"
              placeholder="Filtrar por Sector"
              class="p-mr-2"
              @change="filtradoPorCategoria"
            />
          </div>
        </div>
      </template>
      ...
      <Column field="codigo" header="Código" sortable style="min-width: 9rem">
        <template #body="slotProps">
          ...
          <div>
            <span
              v-for="orden in slotProps.data.ordenes"
              :key="orden.id"
              v-html="
                devuelveCodigoOrdenUrgencias(
                  orden.fecha_fin,
                  orden.prioridad,
                  orden.id,
                  orden.causa_id
      ...
      <div class="grid-container" style="padding-top: 5px">
        ...
        <div class="field" v-if="!causa.id">
          ...
          >
            ...
            />
            &nbsp;
            <Tag value='Se crea con billetera "individual"' severity="info" />
          </div>
        </div>
