...
const loadCausas = async (event?: DataTableSortEvent) => {
  ...
  const response = await causaService.getCausasOrdenPronuncioAbogadoDeLider(opciones)
    ...
    >
      <template #header>
        <div class="flex flex-wrap gap-2 align-items-center justify-content-between">
          <h4 class="m-0">Causas Con ordenes pronunciadas por el abogado</h4>
      ...
      <Column field="codigo" header="Código" sortable style="min-width: 9rem">
        <template #body="slotProps">
          <Button
            ...
            link
            @click="$router.push({ name: 'OrdenesPronuncioAbogadoLider', params: { idCausa: slotProps.data.id } })"
          />
        </template>
      </Column>
