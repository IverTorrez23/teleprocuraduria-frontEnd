...
const loadCausas = async (event?: DataTableSortEvent) => {
  ...
  const response = await causaService.getCausasOrdenPrePresupuestadasDeLider(opciones)
    ...
    >
      <template #header>
        <div class="flex flex-wrap gap-2 align-items-center justify-content-between">
          <h4 class="m-0">Causas Con ordenes Pre Presupuestadas</h4>
      ...
      <Column field="codigo" header="Código" sortable style="min-width: 9rem">
        <template #body="slotProps">
          <Button
            ...
            link
            @click="$router.push({ name: 'OrdenesPrePresupuestadasLider', params: { idCausa: slotProps.data.id } })"
          />
        </template>
      </Column>
